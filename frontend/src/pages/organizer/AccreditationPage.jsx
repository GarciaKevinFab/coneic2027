      ...
      <div className="card p-4 sm:p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <HiQrcode className="w-5 h-5 text-primary" />
            <h2 className="font-display font-bold text-gray-900">Escaner QR</h2>
          </div>
          {!scanActive && (
            <button
              onClick={resetScan}
              className="inline-flex items-center gap-1.5 text-sm text-primary font-medium hover:text-primary-700 transition-colors"
            >
            ...
            <div className="text-center p-6">
              {accreditMutation.isPending ? (
                <>
                  ...
                  <p className="text-gray-600 font-medium">Procesando acreditacion...</p>
                </>
              ) : (
                <>
            ...
            <div className="card border-2 border-green-200 bg-green-50 p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-green-700 text-sm mt-1">{scanResult.message}</p>
                    ...
                    <div className="mt-4 space-y-2">
                      {scanResult.participant.full_name && (
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium text-green-800">{scanResult.participant.full_name}</span>
                        </div>
                      )}
                      {scanResult.participant.university && (
                        <div className="flex justify-between text-sm">
                          <span className="text-green-600">Universidad</span>
                          <span className="font-medium text-green-800">{scanResult.participant.university}</span>
                        </div>
                      )}
                      {scanResult.participant.ticket_type && (
                        <div className="flex justify-between text-sm">
                          <span className="text-green-600">Entrada</span>
                          <span className="font-medium text-green-800 capitalize">{scanResult.participant.ticket_type}</span>
                        </div>
                      )}
                    </div>
                  )}
              ...
              <button
                onClick={resetScan}
                className="w-full mt-5 btn-primary text-sm"
              >
            ...
            <div className="card border-2 border-red-200 bg-red-50 p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-red-700 text-sm mt-1">{scanResult.message}</p>
                </div>
              </div>
